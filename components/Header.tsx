...
const NavItem: React.FC<{ to: string; icon: React.ReactNode; label: string; exact?: boolean; onClick?: () => void }> = ({ to, icon, label, exact = false, onClick }) => {
    const navLinkClasses = ({ isActive }: { isActive: boolean }) =>
        `flex items-center space-x-2 px-4 py-2 rounded-lg transition-colors duration-200 text-sm ${isActive ? 'bg-brand-primary/20 text-white' : 'text-brand-text-secondary hover:bg-brand-surface hover:text-white'}`;

    return (
        <NavLink to={to} className={navLinkClasses} end={exact} onClick={onClick}>
            {icon}
            <span>{label}</span>
        </NavLink>
    );
...
const UserPill: React.FC<{ user: User }> = ({ user }) => {
    return (
        <Link to="/my-account" className="flex items-center space-x-2 bg-brand-surface rounded-full p-1 pr-4 text-sm transition-colors hover:bg-brand-primary/20">
          ...
          <nav className="hidden md:flex items-center space-x-1">
            ...
            <div className="relative" ref={menuRef}>
                <button 
                    ...
                    className="flex items-center space-x-2 px-4 py-2 rounded-lg text-brand-text-secondary hover:bg-brand-surface hover:text-white"
                >
          ...
          <div className="flex items-center space-x-4">
            ...
            {user ? (
              ...
              <div className="hidden md:flex items-center space-x-2">
                <Link to="/signin" className="flex items-center space-x-2 text-sm text-brand-text-secondary hover:text-white transition-colors">
