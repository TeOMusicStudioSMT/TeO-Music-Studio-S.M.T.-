        ...
        <div className="bg-brand-bg min-h-screen">
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
                ...
                <div className="max-w-6xl mx-auto">
                        ...
                        <div className="bg-brand-dark rounded-xl p-8 shadow-2xl shadow-brand-primary/10 space-y-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                    <button onClick={() => handleGenerate('concept')} disabled={loadingStates.concept || user.points < studioCosts.ideaAndLyrics} className="btn-secondary">
                                        <FileTextIcon className="w-5 h-5"/><span>{loadingStates.concept ? '...' : `Idea & Lyrics (-${studioCosts.ideaAndLyrics})`}</span>
                                    </button>
                                    <button onClick={() => handleGenerate('palette')} disabled={!projectData?.generatedIdea || loadingStates.palette || user.points < studioCosts.soundPalette} className="btn-secondary">
                                        <MicIcon className="w-5 h-5"/><span>{loadingStates.palette ? '...' : `Sound Palette (-${studioCosts.soundPalette})`}</span>
                                    </button>
                                    <button onClick={() => handleGenerate('storyboard')} disabled={!projectData?.generatedIdea || loadingStates.storyboard || user.points < studioCosts.videoStoryboard} className="btn-secondary">
                    ...
                    {projectData && !Object.values(loadingStates).some(s => s) && (
